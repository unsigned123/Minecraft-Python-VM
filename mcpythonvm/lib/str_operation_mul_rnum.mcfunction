...
$data modify storage pvm:concat concat.b set from storage $(package_name):$(target_name) stack[-2].data
$execute store result score vm counter run data get storage $(package_name):$(target_name) stack[-1].data
