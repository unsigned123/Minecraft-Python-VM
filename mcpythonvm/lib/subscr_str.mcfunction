data modify storage pvm:subscr subscr_str set value {start:0,end:0}
$execute store result score vm tos run data get storage $(package_name):$(target_name) stack[-1].data
execute store result storage pvm:subscr subscr_str.start int 1 run scoreboard players get vm tos
...
$data modify storage pvm:slicer slicer.str set from storage $(package_name):$(target_name) stack[-2].data
